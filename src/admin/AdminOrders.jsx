...
const AdminOrders = () => {
  ...
  if (loading) {
    return (
      <div className="p-4 xs:p-5 sm:p-6 min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        <div className="flex h-[60vh] items-center justify-center">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 xs:h-16 xs:w-16 border-4 border-purple-500 border-t-transparent mx-auto mb-3 xs:mb-4"></div>
            <div className="text-xl xs:text-2xl font-bold text-purple-200 mb-1 xs:mb-2">Loading Orders...</div>
            <div className="text-purple-400 text-sm xs:text-base">Fetching data from database</div>
          </div>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="p-4 xs:p-5 sm:p-6 min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        <div className="flex h-[60vh] items-center justify-center">
          <div className="text-center bg-red-950/50 p-6 xs:p-8 rounded-xl border border-red-800 max-w-2xl w-full mx-4">
            <div className="text-red-400 text-4xl xs:text-6xl mb-3 xs:mb-4">⚠</div>
            <div className="text-lg xs:text-xl font-bold text-red-300 mb-1 xs:mb-2">Connection Error</div>
            <div className="text-red-400 mb-3 xs:mb-4 text-sm xs:text-base">{error}</div>
            
            {apiErrors.usersError && (
              <div className="bg-red-900/50 p-3 xs:p-4 rounded-lg mb-3 xs:mb-4 text-left">
                <h3 className="font-bold text-red-300 mb-1 xs:mb-2 text-sm xs:text-base">Users API Error:</h3>
                <p className="text-red-400 text-xs xs:text-sm">{apiErrors.usersError}</p>
              </div>
            )}
            
            {apiErrors.ordersError && (
              <div className="bg-red-900/50 p-3 xs:p-4 rounded-lg mb-3 xs:mb-4 text-left">
                <h3 className="font-bold text-red-300 mb-1 xs:mb-2 text-sm xs:text-base">Orders API Error:</h3>
                <p className="text-red-400 text-xs xs:text-sm">{apiErrors.ordersError}</p>
              </div>
            )}
            
            <button 
              onClick={fetchOrders}
              className="px-4 xs:px-6 py-2 bg-red-700 hover:bg-red-600 text-white rounded-lg transition-colors text-sm xs:text-base"
            >
              Retry Connection
            </button>
          </div>
  ...
  return (
    <div className="p-4 xs:p-5 sm:p-6 min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      {/* Notification */}
      {notification.show && (
        <div className={`fixed top-4 right-4 left-4 xs:left-auto z-50 p-3 xs:p-4 rounded-lg shadow-lg border backdrop-blur-sm transition-all duration-300 ${
          notification.type === 'success' 
            ? 'bg-green-900/90 border-green-700 text-green-300' 
            : 'bg-red-900/90 border-red-700 text-red-300'
        }`}>
          <div className="flex items-center gap-2">
            {notification.type === 'success' ? <Check className="w-4 h-4 xs:w-5 xs:h-5" /> : <X className="w-4 h-4 xs:w-5 xs:h-5" />}
            <span className="text-sm xs:text-base">{notification.message}</span>
          </div>
        </div>
      )}
      ...
      {/* API Errors Banner */}
      {(apiErrors.usersError || apiErrors.ordersError) && (
        <div className="mb-4 xs:mb-5 sm:mb-6 p-3 xs:p-4 bg-amber-900/50 border border-amber-700 rounded-lg">
          <div className="flex items-start gap-2 xs:gap-3">
            <div className="text-amber-400 mt-0.5 xs:mt-1">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 xs:h-5 xs:w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
              </svg>
            </div>
            <div>
              <h3 className="text-amber-300 font-medium mb-1 text-sm xs:text-base">Partial API Issues Detected</h3>
              <p className="text-amber-400 text-xs xs:text-sm">
                Some functionality may be limited. The system is running with partial data.
              </p>
              {apiErrors.usersError && (
                <p className="text-amber-400 text-xs xs:text-sm mt-1">Users API: {apiErrors.usersError}</p>
              )}
              {apiErrors.ordersError && (
                <p className="text-amber-400 text-xs xs:text-sm mt-1">Orders API: {apiErrors.ordersError}</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 xs:mb-8">
          <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-3 xs:gap-0 mb-4 xs:mb-6">
            <h1 className="text-2xl xs:text-3xl sm:text-4xl font-black bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
              Orders Management
            </h1>
            <button
              onClick={fetchOrders}
              disabled={refreshing}
              className="flex items-center gap-1 xs:gap-2 px-3 xs:px-4 py-2 bg-purple-700 hover:bg-purple-600 disabled:opacity-50 text-white rounded-lg transition-all duration-200 shadow-lg hover:shadow-purple-700/30 text-sm xs:text-base w-full xs:w-auto justify-center"
            >
              <RefreshCw className={`w-3 h-3 xs:w-4 xs:h-4 ${refreshing ? 'animate-spin' : ''}`} />
              Refresh
            </button>
          </div>

          {/* Stats Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 xs:gap-4 sm:gap-6 mb-6 xs:mb-8">
            <div className="bg-gradient-to-br from-blue-900/40 to-blue-800/20 p-4 xs:p-5 sm:p-6 rounded-lg xs:rounded-xl border border-blue-700/30 backdrop-blur-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-blue-300 text-xs xs:text-sm font-medium">Total Orders</p>
                  <p className="text-2xl xs:text-3xl font-bold text-white">{stats.total}</p>
                </div>
                <ShoppingCart className="w-6 h-6 xs:w-7 xs:h-7 sm:w-8 sm:h-8 text-blue-400" />
              </div>
            </div>
            
            <div className="bg-gradient-to-br from-green-900/40 to-green-800/20 p-4 xs:p-5 sm:p-6 rounded-lg xs:rounded-xl border border-green-700/30 backdrop-blur-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-green-300 text-xs xs:text-sm font-medium">Revenue</p>
                  <p className="text-2xl xs:text-3xl font-bold text-white">${stats.totalRevenue.toFixed(2)}</p>
                </div>
                <DollarSign className="w-6 h-6 xs:w-7 xs:h-7 sm:w-8 sm:h-8 text-green-400" />
              </div>
            </div>
            
            <div className="bg-gradient-to-br from-yellow-900/40 to-yellow-800/20 p-4 xs:p-5 sm:p-6 rounded-lg xs:rounded-xl border border-yellow-700/30 backdrop-blur-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-yellow-300 text-xs xs:text-sm font-medium">Processing</p>
                  <p className="text-2xl xs:text-3xl font-bold text-white">{stats.processing}</p>
                </div>
                <Package className="w-6 h-6 xs:w-7 xs:h-7 sm:w-8 sm:h-8 text-yellow-400" />
              </div>
            </div>
            
            <div className="bg-gradient-to-br from-purple-900/40 to-purple-800/20 p-4 xs:p-5 sm:p-6 rounded-lg xs:rounded-xl border border-purple-700/30 backdrop-blur-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-purple-300 text-xs xs:text-sm font-medium">Delivered</p>
                  <p className="text-2xl xs:text-3xl font-bold text-white">{stats.delivered}</p>
                </div>
                <TrendingUp className="w-6 h-6 xs:w-7 xs:h-7 sm:w-8 sm:h-8 text-purple-400" />
              </div>
            </div>
          </div>

          {/* Filters */}
          <div className="bg-slate-800/50 backdrop-blur-sm p-4 xs:p-5 sm:p-6 rounded-lg xs:rounded-xl border border-slate-700 mb-4 xs:mb-5 sm:mb-6">
            <div className="flex flex-col md:flex-row gap-3 xs:gap-4">
              <div className="flex-1 relative">
                <Search className="absolute left-3 top-3 w-4 h-4 xs:w-5 xs:h-5 text-slate-400" />
                <input
                  type="text"
                  placeholder="Search by customer name, email, or order ID..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-9 xs:pl-10 pr-3 xs:pr-4 py-2.5 xs:py-3 bg-slate-900/50 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 text-sm xs:text-base"
                />
              </div>
              ...
              <select
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value)}
                className="px-3 xs:px-4 py-2.5 xs:py-3 bg-slate-900/50 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 text-sm xs:text-base"
              >
                <option value="All">All Status</option>
                <option value="Processing">Processing</option>
                <option value="Shipped">Shipped</option>
        ...
        {/* Orders Table */}
        {filteredOrders.length === 0 ? (
          <div className="bg-slate-800/30 backdrop-blur-sm rounded-lg xs:rounded-xl border border-slate-700 p-6 xs:p-8 sm:p-12 text-center">
            <Box className="w-12 h-12 xs:w-14 xs:h-14 sm:w-16 sm:h-16 text-slate-600 mx-auto mb-3 xs:mb-4" />
            <h3 className="text-xl xs:text-2xl font-bold text-slate-300 mb-1 xs:mb-2">No orders found</h3>
          ...
          <div className="bg-slate-800/30 backdrop-blur-sm rounded-lg xs:rounded-xl border border-slate-700 overflow-hidden shadow-2xl">
            <div className="overflow-x-auto">
              <table className="min-w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-slate-800 to-slate-700 text-slate-200">
                    <th className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6 text-left font-bold text-xs xs:text-sm">Order ID</th>
                    <th className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6 text-left font-bold text-xs xs:text-sm">Customer</th>
                    <th className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6 text-left font-bold text-xs xs:text-sm">Total</th>
                    <th className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6 text-left font-bold text-xs xs:text-sm">Status</th>
                    <th className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6 text-left font-bold text-xs xs:text-sm">Date</th>
                    <th className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6 text-left font-bold text-xs xs:text-sm">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredOrders.map((order, index) => (
                    <React.Fragment key={order.id}>
                      <tr 
                        ...
                        onClick={() => toggleExpandedOrder(order.id)}
                      >
                        <td className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6">
                          <div className="flex items-center gap-1 xs:gap-2">
                            <div className="w-1.5 h-1.5 xs:w-2 xs:h-2 bg-purple-500 rounded-full"></div>
                            <span className="text-purple-300 font-bold text-xs xs:text-sm">#{order.id}</span>
                          </div>
                        </td>
                        <td className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6">
                          <div className="flex items-center gap-2 xs:gap-3">
                            <div className="w-8 h-8 xs:w-9 xs:h-9 sm:w-10 sm:h-10 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white font-bold text-xs xs:text-sm">
                              {order.userName?.charAt(0)?.toUpperCase() || 'U'}
                            </div>
                            <div className="min-w-0">
                              <p className="text-white font-semibold text-xs xs:text-sm truncate">{order.userName}</p>
                              <p className="text-slate-400 text-xs truncate">{order.userEmail}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6">
                          <span className="text-lg xs:text-xl sm:text-2xl font-bold text-green-400">
                            ${order.total ? order.total.toFixed(2) : 'N/A'}
                          </span>
                        </td>
                        <td className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6">
                          {editingStatus === order.id ? (
                            <div className="flex items-center gap-1 xs:gap-2">
                              <select
                                defaultValue={order.status}
                                className="bg-slate-800 border border-slate-600 rounded-lg px-2 xs:px-3 py-1 text-white text-xs xs:text-sm focus:outline-none focus:border-purple-500"
                                onChange={(e) => {
                                  const newStatus = e.target.value;
                                  updateOrderStatus(order.id, newStatus, order.userId);
                                }}
                              ...
                              <button
                                ...
                                disabled={updatingStatus}
                              >
                                <X size={14} xs:size={16} />
                              </button>
                            </div>
                          ) : (
                            <div className="flex items-center gap-1 xs:gap-2">
                              <span className={`inline-flex items-center gap-1 xs:gap-2 px-2 xs:px-3 py-1 rounded-full text-xs xs:text-sm font-bold ${getStatusBadge(order.status)}`}>
                                {getStatusIcon(order.status)}
                                <span className="hidden xs:inline">{order.status}</span>
                                <span className="xs:hidden">{order.status.substring(0, 3)}</span>
                              </span>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                title="Edit Status"
                              >
                                <Edit3 size={14} xs:size={16} />
                              </button>
                            </div>
                          )}
                        </td>
                        <td className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6 text-slate-400 font-medium text-xs xs:text-sm">
                          {order.date ? new Date(order.date).toLocaleDateString() : 'N/A'}
                        </td>
                        <td className="py-3 xs:py-4 px-3 xs:px-4 sm:px-6">
                          <div className="flex items-center gap-1 xs:gap-2">
                            ...
                            <button
                              ...
                              title={editingStatus === order.id ? "Save Changes" : "Edit Status"}
                            >
                              {editingStatus === order.id ? <Save size={16} xs:size={18} /> : <Edit3 size={16} xs:size={18} />}
                            </button>
                            <div className="text-slate-600">
                              {expandedOrder === order.id ? <ChevronUp size={16} xs:size={20} /> : <ChevronDown size={16} xs:size={20} />}
                            </div>
                          </div>
                        </td>
                      </tr>
                      ...
                      {expandedOrder === order.id && (
                        <tr className="border-b border-slate-700">
                          <td colSpan="6" className="p-0">
                            <div className="bg-gradient-to-r from-slate-800/50 to-slate-700/30 p-3 xs:p-4 sm:p-6">
                              <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-3 xs:gap-0 mb-3 xs:mb-4">
                                <h4 className="text-lg xs:text-xl font-bold text-purple-300 flex items-center gap-1 xs:gap-2">
                                ...
                                <div className="flex items-center gap-2 xs:gap-4">
                                  ...
                                  {editingStatus !== order.id && (
                                    <button
                                      ...
                                      className="flex items-center gap-1 xs:gap-2 px-2 xs:px-3 py-1 bg-purple-700 hover:bg-purple-600 text-white rounded-lg text-xs xs:text-sm transition-colors"
                                    >
                                      <Edit3 size={12} xs:size={14} />
                                      Update Status
                                    </button>
                                  )}
                                </div>
                              ...
                              {editingStatus === order.id && (
                                <div className="mb-4 xs:mb-6 p-3 xs:p-4 bg-slate-900/50 rounded-lg border border-slate-600">
                                  <h5 className="text-base xs:text-lg font-semibold text-white mb-2 xs:mb-3">Update Order Status</h5>
                                  <div className="grid grid-cols-2 xs:grid-cols-4 gap-2 xs:gap-3">
                                    {['Processing', 'Shipped', 'Delivered', 'Canceled'].map(status => (
                                      <button
                                        ...
                                        onClick={() => updateOrderStatus(order.id, status, order.userId)}
                                        disabled={updatingStatus || order.status === status}
                                        className={`flex items-center justify-center gap-1 xs:gap-2 px-2 xs:px-3 py-2 xs:py-3 rounded-lg border transition-all duration-200 text-xs xs:text-sm ${
                                          order.status === status
                                            ? 'bg-purple-700 border-purple-600 text-white cursor-default'
                                            : 'bg-slate-800 border-slate-600 text-slate-300 hover:bg-slate-700 hover:border-purple-500 hover:text-white'
                                        } ${updatingStatus ? 'opacity-50 cursor-not-allowed' : ''}`}
                                      >
                                        ...
                                        {order.status === status && <Check size={14} xs:size={16} />}
                                      </button>
                                    ))}
                                  </div>
                                  <div className="flex justify-end mt-3 xs:mt-4">
                                    <button
                                      onClick={() => setEditingStatus(null)}
                                      className="px-3 xs:px-4 py-1 xs:py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-xs xs:text-sm transition-colors"
                                    >
                                      Cancel
                                    </button>
                                  </div>
                              ...
                              {/* Shipping Address and Order Summary - Side by Side */}
                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 xs:gap-5 sm:gap-6 mb-4 xs:mb-5 sm:mb-6">
                                {/* Shipping Address Section */}
                                {order.shippingAddress && (
                                  <div className="p-3 xs:p-4 sm:p-5 bg-slate-900/50 rounded-lg xs:rounded-xl border border-slate-600 hover:border-purple-500/50 transition-all">
                                    <div className="flex items-center gap-2 xs:gap-3 mb-3 xs:mb-4 pb-2 xs:pb-3 border-b border-slate-700">
                                      <div className="p-1.5 xs:p-2 bg-purple-700/30 rounded-lg">
                                        <Truck className="w-4 h-4 xs:w-5 xs:h-5 text-purple-400" />
                                      </div>
                                      <h5 className="text-base xs:text-lg font-bold text-white">Shipping Address</h5>
                                    </div>
                                    <div className="space-y-2 xs:space-y-3">
                                      <div className="flex items-start gap-2 xs:gap-3">
                                        <div className="w-16 xs:w-20 text-xs xs:text-sm text-slate-400 font-medium">Name:</div>
                                        <div className="flex-1 text-white font-semibold text-sm xs:text-base">{order.shippingAddress.fullName || 'N/A'}</div>
                                      </div>
                                      <div className="flex items-start gap-2 xs:gap-3">
                                        <div className="w-16 xs:w-20 text-xs xs:text-sm text-slate-400 font-medium">Phone:</div>
                                        <div className="flex-1 text-white font-semibold text-sm xs:text-base">{order.shippingAddress.phone || 'N/A'}</div>
                                      </div>
                                      <div className="flex items-start gap-2 xs:gap-3">
                                        <div className="w-16 xs:w-20 text-xs xs:text-sm text-slate-400 font-medium">Address:</div>
                                        <div className="flex-1 text-white font-semibold text-sm xs:text-base">{order.shippingAddress.address || 'N/A'}</div>
                                      </div>
                                      <div className="flex items-start gap-2 xs:gap-3">
                                        <div className="w-16 xs:w-20 text-xs xs:text-sm text-slate-400 font-medium">City:</div>
                                        <div className="flex-1 text-white font-semibold text-sm xs:text-base">{order.shippingAddress.city || 'N/A'}</div>
                                      </div>
                                      <div className="flex items-start gap-2 xs:gap-3">
                                        <div className="w-16 xs:w-20 text-xs xs:text-sm text-slate-400 font-medium">State:</div>
                                        <div className="flex-1 text-white font-semibold text-sm xs:text-base">{order.shippingAddress.state || 'N/A'}</div>
                                      </div>
                                      <div className="flex items-start gap-2 xs:gap-3">
                                        <div className="w-16 xs:w-20 text-xs xs:text-sm text-slate-400 font-medium">Postal:</div>
                                        <div className="flex-1 text-white font-semibold text-sm xs:text-base">{order.shippingAddress.postalCode || order.shippingAddress.zipCode || 'N/A'}</div>
                                      </div>
                                      <div className="flex items-start gap-2 xs:gap-3">
                                        <div className="w-16 xs:w-20 text-xs xs:text-sm text-slate-400 font-medium">Country:</div>
                                        <div className="flex-1 text-white font-semibold text-sm xs:text-base">{order.shippingAddress.country || 'N/A'}</div>
                                      </div>
                                    </div>
                                  </div>
                                )}
                                ...
                                {/* Order Summary Section */}
                                <div className="p-3 xs:p-4 sm:p-5 bg-slate-900/50 rounded-lg xs:rounded-xl border border-slate-600 hover:border-green-500/50 transition-all">
                                    <div className="flex items-center gap-2 xs:gap-3 mb-3 xs:mb-4 pb-2 xs:pb-3 border-b border-slate-700">
                                      <div className="p-1.5 xs:p-2 bg-green-700/30 rounded-lg">
                                        <DollarSign className="w-4 h-4 xs:w-5 xs:h-5 text-green-400" />
                                      </div>
                                      <h5 className="text-base xs:text-lg font-bold text-white">Order Summary</h5>
                                    </div>
                                    <div className="space-y-2 xs:space-y-3">
                                      <div className="flex items-center justify-between p-2 xs:p-3 bg-slate-800/50 rounded-lg">
                                        <span className="text-slate-400 font-medium text-xs xs:text-sm">Order ID:</span>
                                        <span className="text-purple-300 font-bold text-xs xs:text-sm">#{order.id}</span>
                                      </div>
                                      <div className="flex items-center justify-between p-2 xs:p-3 bg-slate-800/50 rounded-lg">
                                        <span className="text-slate-400 font-medium text-xs xs:text-sm">Items Count:</span>
                                        <span className="text-white font-bold text-xs xs:text-sm">{order.items?.length || 0} items</span>
                                      </div>
                                      <div className="flex items-center justify-between p-2 xs:p-3 bg-slate-800/50 rounded-lg">
                                        <span className="text-slate-400 font-medium text-xs xs:text-sm">Order Date:</span>
                                        <span className="text-white font-bold text-xs xs:text-sm">{order.date ? new Date(order.date).toLocaleDateString() : 'N/A'}</span>
                                      </div>
                                      <div className="flex items-center justify-between p-2 xs:p-3 bg-green-900/30 border border-green-700/50 rounded-lg">
                                        <span className="text-green-300 font-medium text-xs xs:text-sm">Total Amount:</span>
                                        <span className="text-green-400 font-bold text-lg xs:text-xl">${order.total ? order.total.toFixed(2) : 'N/A'}</span>
                                      </div>
                                    </div>
                                  </div>
                              </div>
                              ...
                              <div>
                                <div className="flex items-center gap-2 xs:gap-3 mb-3 xs:mb-4">
                                  <div className="p-1.5 xs:p-2 bg-blue-700/30 rounded-lg">
                                    <Box className="w-4 h-4 xs:w-5 xs:h-5 text-blue-400" />
                                  </div>
                                  <h5 className="text-base xs:text-lg font-bold text-white">Order Items</h5>
                                  <div className="flex-1 h-px bg-slate-700"></div>
                                  <span className="text-slate-400 text-xs xs:text-sm">{order.items?.length || 0} items</span>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 xs:gap-4">
                                  {order.items?.map(item => (
                                    <div 
                                      key={item.id} 
                                      className="bg-slate-900/70 backdrop-blur-sm border border-slate-600 rounded-lg xs:rounded-xl p-3 xs:p-4 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/10 transition-all group"
                                    >
                                      <div className="flex items-start gap-3 xs:gap-4">
                                        <div className="w-16 h-16 xs:w-18 xs:h-18 sm:w-20 sm:h-20 bg-slate-800 rounded-lg overflow-hidden flex-shrink-0 border border-slate-700 group-hover:border-blue-500 transition-all">
                                          {item.image ? (
                                            <img
                                              src={item.image}
                                              alt={item.name}
                                          ...
                                          <div className="w-full h-full bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center text-2xl xs:text-3xl">
                                            📦
                                          </div>
                                        </div>
                                        <div className="flex-1 min-w-0">
                                          <h6 className="text-white font-bold text-sm xs:text-base mb-1 xs:mb-2 truncate group-hover:text-blue-300 transition-colors">{item.name}</h6>
                                          <div className="flex items-center justify-between mb-1 xs:mb-2">
                                            <div className="flex items-center gap-1 xs:gap-2">
                                              <span className="text-slate-400 text-xs xs:text-sm">Qty:</span>
                                              <span className="bg-slate-800 px-2 xs:px-3 py-0.5 xs:py-1 rounded-full text-white font-bold text-xs xs:text-sm border border-slate-700">{item.quantity}</span>
                                            </div>
                                            <div className="text-slate-400 text-xs xs:text-sm">
                                              ${item.price ? item.price.toFixed(2) : 'N/A'} each
                                            </div>
                                          </div>
                                          <div className="flex items-center justify-between pt-1 xs:pt-2 border-t border-slate-700">
                                            <span className="text-slate-400 text-xs xs:text-sm font-medium">Subtotal:</span>
                                            <span className="text-green-400 font-bold text-base xs:text-lg">
                                              ${(item.price * item.quantity).toFixed(2)}
                                            </span>
                                          </div>
                                        </div>
                                    ...
                                    <div className="col-span-full text-center py-6 xs:py-8 bg-slate-900/50 rounded-lg xs:rounded-xl border border-slate-700">
                                      <Package className="w-10 h-10 xs:w-12 xs:h-12 text-slate-600 mx-auto mb-2 xs:mb-3" />
                                      <p className="text-slate-500 font-medium text-sm xs:text-base">No items found for this order</p>
                                    </div>
                                  )}
                                </div>
                              </div>

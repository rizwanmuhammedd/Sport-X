...
// FIXED VERSION - guests can browse without login modal
import axios from "axios";
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // ✅ KEY FIX: If user has no token, they are a guest.
    // Don't attempt refresh, don't show modal — just silently reject.
    const token = localStorage.getItem("token");
    if (status === 401 && !token) {
      return Promise.reject(error);
    }
